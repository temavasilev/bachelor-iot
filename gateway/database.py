...
"""
    ...
    def add_device(self, device_id, jsonpath, topic):
        ...
        try:
            ...
            self.cursor.execute(
                """INSERT INTO devices (id, device_id, jsonpath, topic) VALUES (%s, %s, %s, %s)""",
                (device_uuid, device_id, jsonpath, topic),
            )
    ...
    def get_jsonpath(self, device_id, topic):
        ...
        try:
            self.cursor.execute(
                """SELECT jsonpath FROM devices WHERE device_id=%s AND topic=%s""",
                (device_id, topic),
            )
    ...
    def get_topic(self, device_id):
        ...
        try:
            self.cursor.execute(
                """SELECT topic FROM devices WHERE device_id=%s""", (device_id,)
            )
            return self.cursor.fetchone()
        except TypeError:
            print(f"Device {device_id} does not exist!")
    ...
    def get_device_id(self, jsonpath, topic):
        ...
        try:
            self.cursor.execute(
                """SELECT device_id FROM devices WHERE jsonpath=%s AND topic=%s""",
                (jsonpath, topic),
            )
            return self.cursor.fetchone()
        except TypeError:
    ...
    def delete_device(self, device_id, topic):
        ...
        try:
            self.cursor.execute(
                """DELETE FROM devices WHERE device_id=%s AND topic=%s""",
                (device_id, topic),
            )
            self.connection.commit()
        except psycopg2.IntegrityError:
    ...
    def update_device(self, jsonpath, topic, device_id):
        ...
        try:
            self.cursor.execute(
                """UPDATE devices SET jsonpath=%s, topic=%s WHERE device_id=%s""",
                (jsonpath, topic, device_id),
            )
            self.connection.commit()
        except psycopg2.IntegrityError:
            print(f"Device {device_id} does not exist!")
